...
def baby_shark(x, y, size, feed):
    ...
    while q:
        ...
        for i in range(equal_length):
            ...
            for j in range(4):
                tx, ty = nx + dr[j], ny + dc[j]
                if 0 <= tx < n and 0 <= ty < n and not visited[tx][ty] and arr[tx][ty] <= n_size:
                    visited[tx][ty] = 1
                    if arr[tx][ty] == n_size or arr[tx][ty] == 0:
                        q.append((tx, ty, n_size, n_feed, move+1))
                    else:
                        if not found:
                            success_x, success_y = tx, ty
                            found = True
                        else:
                            if tx < success_x or (tx == success_x and ty < success_y):
                                found_move = move
                                arr2[nx][ny] = (cnt, n_feed, n_size)
                                success_x, success_y = tx, ty
        # 찾았던 경우 배열 중에서 x축 작은순, y축 작은 순으로 배치
        if found:
            # success.sort(key=lambda xx: (xx[0], xx[1]))
