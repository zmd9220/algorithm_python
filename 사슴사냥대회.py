...
def Me(opp, turn, opp_prev, opp_last_pattern):
    def Me(opp, turn, opp_prev, opp_last_pattern):
        if opp_last_pattern[pattern_count[opp]][turn] != -1:
            opp_last_pattern_this_turn = opp_last_pattern[pattern_count[opp]][turn]
            if opp_last_pattern_this_turn == DEER:
                if opp_prev == DEER:
                    return RABBIT
                else:
                    return DEER
            elif opp_last_pattern_this_turn == RABBIT:
                if opp_prev == DEER:
                    return SNAKE
...
def Opponent1(opp, turn, opp_prev, opp_last_pattern):
    print('opp1', opp, opp_last_pattern[pattern_count[opp]])
    return 0

def Opponent2(opp, turn, opp_prev, opp_last_pattern):
    print('opp2', opp, opp_last_pattern[pattern_count[opp]])
    print(opp_prev)
    if turn == 0:
        return 1
    return opp_prev
...
def Opponent3(opp, turn, opp_prev, opp_last_pattern):
    print('opp3', opp, opp_last_pattern[pattern_count[opp]])
    if turn == 0:
        return 2
    return opp_prev
